// This is a fork of http2.Transport and the associated code.
//
// The purpose of this is to expose the data frames so that we can handle
// streams directly. The RoundTripper implementation has thus changed.
//
...
// listen sends a request, gets the headers of the response, and then listens
// for additional data frames.
//
// Data frames are then shunted onto the data channel.
//
// When the remote indicates that the transaction is complete, the channel is
// closed.
func (cc *clientConn) listen(req *http.Request) (*http.Response, *clientStream, error) {
	cc.mu.Lock()
	...
	if re.err != nil {
		//return nil, re.err
		fmt.Printf("Error closing client stream: %s", re.err)
	}
	res := re.res
	res.Request = req
	res.TLS = cc.tlsState
